...
from pyanaconda.modules.storage.constants import InitializationMode, StorageState
...
class DiskInitializationModule(KickstartBaseModule):
    ...
    def on_storage_changed(self, storage, state):
        ...
        # Do nothing the partitioning is not applied.
        if state is not StorageState.PARTED:
            return

        # Do nothing if the mode is not set to CLEAR_NONE.
        if not self.initialization_mode != InitializationMode.CLEAR_NONE:
            return
    ...
    def _find_cleared_devices(self, storage):
