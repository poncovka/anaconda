...
from pyanaconda.core.storage import PARTITION_ONLY_FORMAT_TYPES, SUPPORTED_DEVICE_TYPES, \
...
def bound_size(size, device, old_size):
    ...
    if not size:
        if max_size:
            ...
            log.warning("No size specified and no maximum size "
                        "available, setting size back to original "
                        "size (%d).", old_size)
            size = old_size
    else:
        if max_size:
            if size > max_size:
                log.warning("Size specified (%d) is greater than "
                            "the maximum size for this device (%d), "
                            "using maximum size.", size, max_size)
                size = max_size
        else:
            log.warning("Unknown upper bound on size. Using requested "
                        "size (%d).", size)

        if size < min_size:
            log.warning("Size specified (%d) is less than the minimum "
                        "size for this device (%d), using minimum size.",
                        size, min_size)
            size = min_size
