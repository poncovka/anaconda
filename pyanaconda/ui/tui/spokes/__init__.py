...
from pyanaconda.i18n import N_, C_, _
...
class NormalTUISpoke(TUISpoke, NormalSpoke):
    ...
    def input(self, args, key):
        ...
        if key.lower() == C_("TUI|Spoke Navigation", "h"):
            if self.has_help:
        ...
        return TUISpoke.input(self, args, key)
...
class EditTUIDialog(NormalTUISpoke):
    ...
    def prompt(self, entry=None):
        ...
        if entry.aux == self.PASSWORD:
            ...
            return tui.Prompt(_("Enter a new value for '%s' and press %s") % (entry.title, tui.Prompt.ENTER))
