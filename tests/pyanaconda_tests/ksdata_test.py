...
class BlivetTestCase(unittest.TestCase):
    ...
    def test_bootloader_in_kickstart(self):
        ...
        # prepboot test case
        with patch('pyanaconda.storage.InstallerStorage.bootloader_device', new_callable=PropertyMock) as mock_bootloader_device:
            with patch('pyanaconda.storage.InstallerStorage.mountpoints', new_callable=PropertyMock) as mock_mountpoints:
        ...
        # biosboot test case
        with patch('pyanaconda.storage.InstallerStorage.devices', new_callable=PropertyMock) as mock_devices:
            with patch('pyanaconda.storage.InstallerStorage.mountpoints', new_callable=PropertyMock) as mock_mountpoints:
